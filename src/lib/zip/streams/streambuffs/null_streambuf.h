...
protected:
    ...
    std::streamsize xsgetn(char_type* s [[maybe_unused]], std::streamsize count [[maybe_unused]]) override {
        return 0;
    }
    
    std::streamsize xsputn(const ELEM_TYPE* ptr [[maybe_unused]], std::streamsize count) override {
        return count;
    }
